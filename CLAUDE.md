...
**`describe-table` answers with both directions of the foreign keys, and the two are filtered differently on purpose.** `foreignKeys` (declared *on* the table) lists every key including ones pointing outside the allowlist, flagged `reachable: false` — the referenced name is in `pg_get_constraintdef`'s text, which this tool has always returned, so nothing new is disclosed and a model is spared a `describe-table` that will refuse. `referencedBy` (declared on *other* tables, pointing here) lists only reachable ones, because an out-of-scope row there would be a table name this token was never granted, handed over by describing one it was; what is reported instead is `referencedByOutOfScope`, a count. Counted rather than silent, for the reason `protocolMessagesExcluded` exists: "nothing references this table" and "I may not tell you what does" are different answers. `reachable` is the allowlist predicate **and** `has_table_privilege`, since a granted name the role cannot `SELECT` is still not reachable — verified against a least-privilege reader, which is the deployment the README's `GRANT` recipe produces.

Three other things `describe-table` reads that the obvious query misses. **CHECK constraints are a column's value domain** as much as an enum type is — `CHECK (status IN ('open','closed','void'))` on a `text` column — so `contype` collects `c` alongside `p` and `u`; a filter on a value outside them returns no rows, which reads as an empty result rather than a mistake. **An enum column's labels are three hops from `atttypid`**: the type itself, an array's `typelem`, or a domain's `typbasetype`, because `order_status[]` and a domain over `order_status` are as enumerated as the bare column and reading only `atttypid` reports neither. And **a view's `pg_get_viewdef` is its relationship documentation** — a view has no primary key, no foreign keys and (unmaterialized) no indexes, so without it a view describes as a bag of columns joined to nothing; it is clipped at `MAX_VIEW_DEFINITION` because a view body is arbitrary user SQL with no natural bound and this tool is called before most queries. All four queries still go out in one `Promise.all` on `target.oid`, so the round-trip cost is unchanged.
...
`listAccountMessages()` is the same shaping over **every** chat — the dashboard's message table. It reaches that by leaving `jid` off `MessagePageOptions`, which is now optional, and adds `terms` (matched against the body expression `searchMessages` uses, extracted as `messageBodyJoin` so the two cannot drift) and `order`. **MCP cannot reach that branch:** every tool goes through `listMessages()`, which takes `remoteJid` as a required positional argument, and `instanceId` is unconditional in the SQL either way. `listTelegramMessagesPage()` has the same optional `chatId` for the same caller and the same reason. Names on an account-wide page come from the contact and group caches only — no per-group participant pass, which a hundred-row page spanning a hundred groups would otherwise pay for.
...
**The chat listing is now walked once into a cache, and every caller reads pages off that.** `findChats` takes no search term and no ordering, so a table that searches and sorts the *whole* account cannot push either down to Evolution. `chatListing()` in `chats.ts` walks the endpoint to the end (2000 a page, capped at 10 000 rows), names the rows once, and memoises the result for **60 seconds** — TTL plus in-flight dedupe plus successes-only, the same shape `contactDirectory()` has. `listChatsPage()` filters, sorts and slices it for the dashboard table; `listChats({ take, skip })` keeps its old signature and return shape for the scope picker and the `list-chats` tool and is a slice of the same listing. That is strictly cheaper than what it replaced: naming a page cost Evolution's entire contact table, and it was paid once per page while walking `skip`.

Two flags come out of that walk and must not be merged. `truncated` means the cap was hit — the account holds more than the listing will hold. `incomplete` (`failed` inside the module) means Evolution could not be asked for all of it. Neither is `hasMore`, which is only "there is another page of what I have", and offering a next page for either of the other two fetches nothing. The old first-page-swallowed/later-page-rethrown rule survives in `listChats`, which still refuses `skip > 0` on an incomplete listing rather than letting a short one read as the end of the list.
...
**Invitation email is an extra delivery, never the only one.** The link is still shown once at creation; `POST /api/org/invites/:id/email` mails it from that dialog, and is authorized by *presenting the code* — the server resolves it back to the row, checks id and organization, builds the URL itself and mails only the stored address, so nothing in the request becomes mail content or a recipient. The pending table cannot re-send a link (only the hash exists), so `POST /api/org/invites/:id/resend` supersedes it through `createInvite()` and mails the new one — and answers **200 with the new link when the mail fails**, because the old link is already dead and an invitation nobody has a link for is orphaned. Its throttle check runs *before* the re-mint for the same reason. `assertInviteMailAllowed()` is an in-memory speed bump (per address per minute, per organization per hour), not a control: every signup is an admin of some organization, so without it the form mails anyone from this deployment's domain with a user-chosen organization name in the subject.
...
`convert.ts` maps TL objects to rows and does no I/O; `store.ts` is every write; `syncer.ts` decides when. **Four sources, because none is complete.** Live updates, applied through one queue in arrival order. Catch-up on connect: teleproto's common difference from the persisted `sessions.update_*`, then **our own** `updates.getChannelDifference` per channel from `chats.channel_pts` — teleproto only tracks channels it saw an update for in the current process, so after a restart it would miss every channel's offline edits and deletions. A dialog pass on connect and every ten minutes, filling a chat forward when its top message is newer than what is stored; that is what covers channels Telegram stopped pushing to a client nobody was polling. And backfill of older history. The update state must be seeded **before** `connect()` (teleproto fetches a fresh one on connect only when it has none, and a fresh one skips the downtime), and the raw handler attached before `catchUp()`, or the difference is dispatched to nobody.

**The same message arrives from several sources in no guaranteed order**, so every write is an idempotent upsert, and **a tombstone is never resurrected**: a deletion clears `text` and `entities` and sets `deleted_at` (a CHECK enforces the pair), and every message upsert is conditional on `deleted_at IS NULL`. Retaining text someone deleted for everyone, and serving it to a model, is the thing this prevents. Edits overwrite in place — there is no second row, unlike WhatsApp — and reactions are an aggregate on the message (`[{emoji, count}]`), never rows of their own.

**What could not be recovered is recorded, not hidden.** `chats.gap_since` is set when a forward fill runs past its page cap or Telegram answers `ChannelDifferenceTooLong`; `backfill_stopped` says why history ends where it does, and only `start` (with `backfill_complete`) means the chat's first message was reached — `days`, `count`, `skipped` and `inaccessible` all mean older history exists and was not fetched. A read tool must surface both rather than present a partial chat as a whole one.

**Backfill is slow on purpose**: newest chat first, one page of 100 at a time spaced by `TELEGRAM_BACKFILL_PAGE_DELAY_MS`, capped per chat by count and age, broadcast channels skipped by default, and a flood wait pauses the whole session's backfill. Accounts on an unofficial client are watched, and one that hammers history is how an account gets restricted. No takeout sessions: they need confirming on the phone and usually impose a day's delay.

**A chat row exists only for a chat with messages.** An entity that merely passes by — a forward's origin, a group member, a resolved username — refreshes a known chat or a `users` row and never creates a chat, or the listing fills with channels someone once forwarded from.
...
**The dashboard's three tables search, sort and page on the server.** Chats, Messages and Contacts each open a dialog that holds no pool: it asks for one page of 100 and renders what it is given, and `q`, `sort`, `dir` and `page` are request parameters. Doing any of it in the browser would mean a search that answers about the rows that happened to be fetched rather than about the account — which is what the chats dialog used to do, over a capped 2000. `useTablePage()` owns the shared state (a generation counter so a superseded response cannot land, a page reset on every query change, and `hasMore` left alone by a failed page). The one thing the server cannot order is a *name* on the messages table: senders and chats are named after the query, so only Time is sortable there, and the other columns carry no sort control rather than a page-local one.
