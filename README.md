...
want WhatsApp connections** — they sit behind a compose profile, and the
`NUXT_EVOLUTION_*` variables are optional.
...
- The scope editor no longer lists `search-messages` when the database URL is
  unset. It used to, because it read the raw tool registry; it now filters by the
  connection's kind and by each tool's prerequisites, so a checkbox that would
  grant nothing is not offered.
